...
| AI | **Google Gemini** (`gemini-3.1-pro-preview`), behind a provider seam | Amended. Was Anthropic Claude, which is the better-evaluated choice for this product and remains one environment variable away; Gemini is roughly half the output cost and the classifier — the highest-volume call — can eventually run ten times cheaper. Both adapters are complete. See section 4 and decision 13. |
...
4. **No server-side model fallback on refusal.** App-owned copy is safer than a second model. *Extended on the Gemini migration:* a provider **block** is now tracked separately from a refusal. Both still end in reviewed copy, so the rule is unchanged — but they are different events. A refusal means the model saw the message and declined; a block means a provider filter intercepted it and the model never saw it. The second is an operational alarm, and folding it into the first would hide it.
...
13. **Gemini primary, Anthropic retained behind a seam.** The move is a cost decision: Gemini 3.1 Pro is $2/$12 per million against Opus 5's $5/$25, and the classifier runs on every message. Anthropic was *not* deleted — a provider seam whose second implementation has been removed is not a seam, it is a comment, and the ability to move the whole application back with one environment variable is the cheapest insurance available against a migration that reads badly to the people using it. A test asserts that reversal still works.
14. **Google's content filters are set to `OFF`, and that is the safer setting here.** This is the line in the codebase most likely to be challenged, so the reasoning is in `google/client.ts` at length. Briefly: Aeris already has four tested safety layers — deterministic floors, a classifier that can only raise risk, reviewed copy, and eight live-stream output guards. Google's filter is a fifth, calibrated for general-purpose content, that (a) hides self-harm text from the classifier whose only job is to read it, and (b) truncates replies mid-sentence to distressed people, where Aeris's own guards discard the draft and substitute reviewed words. Google's non-adjustable core protections still apply, which is why `blocked` remains a handled failure kind.
15. **The classifier does not get the cheap model by default.** `gemini-3.1-flash-lite` is ~10× cheaper and is the single largest saving available. It is gated behind `npm run eval:safety`, and a unit test asserts the default is not a Lite model — so removing the gate is a visible change, not a quiet one.
16. **The safety evaluation is a command, not a sentence.** Decision 4 and §6 both required re-running an evaluation on a model change, and no evaluation existed. `npm run eval:safety` now runs 20 fixtures — including every case the deterministic layer previously got wrong — and fails on a single miss against the crisis set.
17. **`classifierVersion()` is derived from the model.** A safety event recording only "classifier 1.0.0" cannot answer the question that matters after an incident. Composing the version from the rules version and the model id means a model swap cannot ship without the audit trail changing with it, and nobody has to remember to bump anything.
